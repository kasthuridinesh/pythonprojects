...
class Books(Validation):
    ...
    books_logger.info("verification by id started")

    def verify_book_by_id(self):

        """
        Storing all constants data in the json
        randomly generating id by using its end point
        Verifying integer data and sting data
        """
        global data_item
        endpoint = GET_BOOK_BY_ID_ENDPOINT
        book_id = test_utils.get_random_number(1, 200)

        book_by_id = str(book_id)
        book_by_id_endpoint = f'{endpoint}{book_by_id}'
        book_by_id_final = requests.get(self.base_url + book_by_id_endpoint)
        single = book_by_id_final.json()
        print(single)

        json_data = json.dumps(single)
        json_data_for_id = json.loads(json_data)
        actual_response_data = json_data_for_id[APIKeysConstants.ID_KEY]
        self.books_logger.info(actual_response_data)

        books_dict = test_data_handler.get_book_by_id(book_id)
        self.books_logger.info(books_dict)
        expected_response_data = books_dict[APIKeysConstants.ID_KEY]
        books_verify.verify_number(self, actual_response_data, expected_response_data, InfoMessages.VERIFY_ID_MESSAGE)

        # verification of  title string
        actual_response_data = json_data_for_id[APIKeysConstants.TITLE]
        self.books_logger.info(actual_response_data)
        expected_response_data = books_dict[APIKeysConstants.TITLE]
        books_verify.verify_string(self, actual_response_data, expected_response_data,
                                   InfoMessages.VERIFY_TITLE_MESSAGE)

        # verification of  description string
        actual_response_data = json_data_for_id[APIKeysConstants.DESCRIPTION]
        self.books_logger.info(actual_response_data)
        expected_response_data = books_dict[APIKeysConstants.DESCRIPTION]
        books_verify.verify_string(self, actual_response_data, expected_response_data,
                                   InfoMessages.VERIFY_TITLE_DESCRIPTION)

    def verify_get_books(self):
        """
    ...
    def verify_get_book_detail_by_id(self):
        """
    ...
    def verify_update_book_details_by_id(self):
        """
    ...
    def verify_delete_book_by_id(self):
        """
